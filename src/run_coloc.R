...
files <- list.files("./reports/coloc", pattern="harmonized\\.csv", full.names=TRUE)
results <- list()

for (f in files) {
  d <- fread(f)
  gwas <- list(beta=d$beta_gwas, varbeta=(d$standard_error^2), N=NA, MAF=d$maf, type="cc", s=0.5, snp=d$variant_id)
  qtl  <- list(beta=d$slope, varbeta=(d$slope_se^2), N=NA, MAF=d$maf, type="quant", snp=d$variant_id)
  res <- tryCatch(coloc.abf(dataset1=gwas, dataset2=qtl), error=function(e) NULL)
  if (!is.null(res)) {
    s <- res$summary
...
write.csv(resdf, "./reports/coloc/coloc_results.csv", row.names=FALSE)
